...
function submitForm() {
    ...
    let height = dimensions[parseInt(perconf.width)];
    let idPianoDay = "0,1,2,3,4";
    ...
    let elmt = document.getElementById('idTree');
    for (let i = 0, j = 0; i < elmt.options.length; ++i) {
    ...
    document.cookie = conf.COOKIE_NAME + "=" + encodeURIComponent(JSON.stringify(perconf)) + expires + "; path=/; SameSite=Strict";
...
function reloadImage() {
    loadImage(document.getElementById('img_planning').src)
}

function loadImage(url) {
    let img_planning = new Image();
    ...
    img_planning.addEventListener('error', function () {
        document.getElementById('href_planning').removeAttribute("href");
        document.getElementById('img_planning').src = url;//'img/error.png';
        //document.getElementById('img_planning').src = url;
